...
database_name = "media_platform_database"
...
class Table:
    def __init__(self, tablename, data={}, commit=True):
        ...
        for d in columns:
            if not d in self.data.keys():
                default = get_record_default_value(database_name, tablename, d)
    ...
    def save(self, fields=None):
        ...
        if fields is None:
            for k,v in get_column_names(self.tablename):
                if k == "id": continue
                if type(v) in typeswithquotes: query+=f" {k} = '{v}',"
                else: query+=f" {k} = {v},"
        else:
